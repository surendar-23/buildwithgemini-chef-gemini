...
def a2ui_callback(
    ...
    for part in llm_response.content.parts:
        ...
        if not _surface_is_renderable(messages):
            # Extract literal strings from components so user sees the information instead of an error
            extracted_texts = []
            for m in messages:
                su = m.get("surfaceUpdate")
                if isinstance(su, dict) and su.get("components"):
                    for comp in su["components"]:
                        c_data = comp.get("component", {})
                        text_comp = c_data.get("Text", {})
                        lit = text_comp.get("text", {}).get("literalString")
                        if lit:
                            extracted_texts.append(lit)
            if extracted_texts:
                clean_text = "\n".join(extracted_texts)
            else:
                clean_text = _TAG_RE.sub("", text).strip()
                if not clean_text or clean_text.startswith("{") or clean_text.startswith("["):
                    clean_text = "Here is the information requested: Please ask if you need further details or a specific recipe summary!"
            return LlmResponse(
